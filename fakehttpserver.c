		...
		if(fork()==0)
		{
			if(fork()==0)
			{
				...
				for(int i=1;i<=5;i++)
				{
					...
					{
						printf("Cant able to connect to 55 port\n");
						close(hostsock);
						sleep(1);
						continue;
					}
				...
				if(strcmp(hostname,"dontknow")==0)
				{
					printf("\nHostname is dontknow\n");
					close(connfd);
					exit(0);
				}
				printf("Finaallyy found hosybame wjich is %s\n",hostname);
				if(connect(proxysock,(struct sockaddr*)&proxyservaddr,sizeof(proxyservaddr))!=0)
				{
					printf("Cant connectc to proxyserver\n");
					exit(-1);
				}
				for( ; ; )
				{
