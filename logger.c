...
struct node* delnode(struct node *data,int port,char *hostname)
{
	if(data==NULL)
	{
		printf("delnode:data is NULL\n");
		strcpy(hostname,"dontknow");
		return NULL;
	}
	if(data->next==NULL)
	{
		printf("delnode:data->next is NULL\n");
		if(data->port==port)
		{
	...
	if(data->port==port)
	{
		printf("Found source port entry in data->port delteing head and returning back data->next\n");
		strcpy(hostname,data->hostname);
	...
	for( ; temp2!=NULL ;temp2=temp2->next )
	{
		if(temp2->port==port)
		{
			printf("found source port registry in linked list\n");
...
int main()
{
	...
	for( ; ; )
	{
		...
		if(FD_ISSET(fd,&rset))
		{
			printf("Call from dmesg\n");
			int n=read(fd,fdbuffer,500);
			fdbuffer[n]='\0';
			printf("FD Buffer is %s\n",fdbuffer);
